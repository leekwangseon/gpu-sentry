...
generate_html_report() {
    local max_temp avg_temp max_power avg_gpu avg_mem key value level event cause action
    ...
    avg_gpu=$(metric_avg 8); avg_mem=$(metric_avg 9)
    {
        ...
        printf '<h2>Summary</h2><div class="cards"><div class="card"><b>GPUs</b><br>%s</div><div class="card"><b>Driver</b><br>%s</div><div class="card"><b>CUDA</b><br>%s</div><div class="card"><b>CUDA path</b><br>%s<br><small>%s</small></div><div class="card"><b>Max temperature</b><br>%s &deg;C</div><div class="card"><b>Avg temperature</b><br>%s &deg;C</div><div class="card"><b>Max power</b><br>%s W</div><div class="card"><b>Avg GPU util</b><br>%s%%</div><div class="card"><b>Avg memory util</b><br>%s%%</div></div>\n' "$GPU_COUNT" "$(html_escape "$DRIVER_VERSION")" "$(html_escape "$CUDA_VERSION")" "$(html_escape "$CUDA_HOME_DETECTED")" "$(html_escape "$CUDA_SOURCE")" "$max_temp" "$avg_temp" "$max_power" "$avg_gpu" "$avg_mem"
