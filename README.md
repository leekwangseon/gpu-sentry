...
`gpu-doctor`는 NVIDIA GPU 서버를 현장에서 진단하기 위한 로컬 및 SSH 기반 도구입니다.
...
제조사 수집기는 `vendor.log`에 안내 메시지를 기록합니다. gpu-doctor는 BIOS나 BMC
설정을 변경하지 않습니다.
...
gpu-doctor는 진단 정보를 읽고 필요할 때 gpu-burn을 설치하지만, 펌웨어 업데이트나
